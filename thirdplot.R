...
shinyApp(
  ...
  server <-  function(input, output) {
    ...
    output$map <- renderLeaflet({
      ...
      map1=leaflet(dat()) %>%
        setView(lng=cen_lon, lat=cen_lat, zoom=12) %>%
        addTiles() %>%
    ...
    output$priceplot1 <- renderPlot({
      ...
      ggplot(dataset,aes(x=room_type, y=avg_price))+geom_point(colour = "red", size = 3) +
        xlab("room type") +
        ylab("average price")+theme_bw()
    })
    output$priceplot2 <- renderPlot({
